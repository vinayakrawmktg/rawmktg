...
A(sec("15","rawmktg","How is RawMktg's approach different, and who is it for?",
      "RawMktg is not a tracker. It is a diagnosis-and-remediation engine: it crawls your pages the way an AI crawler does, measures Share of Model, and returns located, prioritised findings and the fix, not just a score.",
      "So it is complementary to every tracker above, run one to learn your position, run RawMktg to change it. (Disclosure: this is our product; the comparison below is kept as fair as we can make it.)"))
A(p("The trackers in this guide answer one question well: am I cited, how often, and versus whom. RawMktg is built for the question that comes next, why am I not, and what exactly do I change. It fetches each page as OAI-SearchBot or PerplexityBot would, computes the "+L("Content Visibility Ratio","/features/ai-visibility-audit")+" (how much of the page survives without JavaScript), runs roughly fifty checks across retrievability, answer structure and markup, and returns findings located to the exact page and element and weighted by impact, alongside the generated fix. It measures "+L("Share of Model","/features/share-of-model")+" the way a tracker measures visibility, but it does not stop at the number."))
A(h3("The benefit of closing the loop"))
A(p("A tracker's dashboard tells you the temperature is falling; it does not tell you the window is open. Because RawMktg diagnoses and remediates in one workflow, the output is not a trend line but a prioritised worklist: this page fails gate 1 on a stray noindex, that section has no answer block, this comparison page is missing schema, ranked by the citations each fix is likely to move. The path from measure, to why, to fix lives in one place, which is the difference between knowing you have a problem and having it solved. It closes exactly the gap every tracker's own customers name in their reviews: monitoring-only."))
A(h3("Who this approach is for"))
A(p("Diagnosis-and-remediation fits teams that have already looked at a tracker's number and asked 'now what': in-house B2B marketers who need to move the number rather than watch it; SEO and content teams who want located fixes instead of a score; and agencies who need to show clients the change, not just the chart. It is less relevant if all you need today is a broad multi-engine monitor, in which case a tracker above, paired with RawMktg when you are ready to act, is the right stack."))
A(p("Here is how the approaches stack up. The trackers lead on breadth of engine monitoring, which is their job; RawMktg leads on the audit-to-fix axis, which is a different job. Read it as complementary, not head-to-head."))
A(table("Trackers vs RawMktg, feature by feature. The trackers measure; RawMktg diagnoses and fixes. (Yes / Partial / No, September 2026.)",
...
A(p("The honest summary: if the question is 'where do I stand across the AI engines', buy a tracker. If the question is 'why am I not cited and what do I change', that is the "+L("audit-and-fix job RawMktg is built for","/features/findings-and-fixes")+", and you can "+L("start with a free audit of 25 pages","/pricing")+". Most serious programs end up wanting both."))

A(sec("16","cost","What actually drives the price?",
...
A(sec("17","which","Which AI visibility tool should you buy?",
...
A(sec("18","run","How do you actually run one once you have it?",
...
A(sec("19","method","Method and honest limits",
...
toc=('<li><a href="#what"><span class="toc-num">01</span>What the tools do</a></li>'
     ...
     '<li><a href="#rawmktg"><span class="toc-num">15</span>The RawMktg approach</a></li>'
     '<li><a href="#cost"><span class="toc-num">16</span>What drives the price</a></li>'
     '<li><a href="#which"><span class="toc-num">17</span>Which to buy</a></li>'
     '<li><a href="#run"><span class="toc-num">18</span>How to run one</a></li>'
     '<li><a href="#method"><span class="toc-num">19</span>Method &amp; limits</a></li>')
